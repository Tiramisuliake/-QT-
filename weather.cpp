...
Weather::Weather(QWidget* parent) : QWidget(parent), ui(new Ui::Weather)
{
    ...
    mExitAct->setText(tr("返回"));
    mExitAct->setIcon(QIcon(":/res/close.png"));  // 假设有一个返回图标
    mExitMenu->addAction(mExitAct);

    connect(mExitAct, &QAction::triggered, this, &Weather::backToMainWindow);
...
void Weather::backmain()
{
    //timer->stop();
    emit backWindow3();
}

void Weather::backToMainWindow()
{
    // 处理返回到主窗口的逻辑
    MainWindow* mainWindow = new MainWindow();  // 创建主窗口对象
